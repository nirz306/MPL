...
;arithmetic operations(+,/,-,*) using suitable macros.Define procedure for each operation.
;-----------------------------------------------------------
%macro IO 4
  mov rax,%1
  mov rdi,%2
...
section .data
  ...
  choice resb 2
  answer resq 1
  n1 resq 1 ;16byte + 1byte for enter = 17 byte i/p
  n2 resq 1 ; 16byte + 1byte for enter = 17 byte i/p
  temp_1 resq 1
  temp_2 resq 1
...
_start:
  IO 1,1,m2,l2 ;******MENU********
  IO 1,1,m1,l1 ;enter your choice
  IO 0,0,choice,2 ; input your choice
